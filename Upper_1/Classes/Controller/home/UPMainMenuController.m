...
- (void)switchController:(NSInteger)index
{
    ...
    if (index!=self.selectedIndex) {
        [self transitionFromViewController:self.childViewControllers[self.selectedIndex] toViewController:self.childViewControllers[index] duration:0.1 options:UIViewAnimationOptionAutoreverse animations:nil completion:^(BOOL finished) {
            if (finished) {
